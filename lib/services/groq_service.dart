...
class GroqService {
  ...
  Future<String> transcribe(String filePath) async {
    ...
    appLog('GroqService: API Key available: ${apiKey != null}');
    if (apiKey == null) {
    ...
    try {
      ...
      appLog('GroqService: 送信ファイルサイズ: $fileSize bytes');

      if (fileSize < 100) {
        throw Exception('録音データが空に近い（$fileSize bytes）ため、文字起こしを中断しました。');
      }

      appLog('GroqService: Groq APIへアップロード中 ($filePath)...');
      ...
      appLog('GroqService: レスポンスステータス: ${response.statusCode}');
      if (response.statusCode == 200) {
        final text = response.data['text'] ?? '';
        appLog('GroqService: 文字起こし成功。文字数: ${text.length}');
        return text;
      } else {
        appLog('GroqService: APIエラーレスポンス: ${response.data}');
        final errorMsg =
            response.data['error']?['message'] ?? response.statusMessage;
        throw Exception('Groq APIエラー ($errorMsg)');
      }
    } catch (e) {
      appLog('GroqService: 例外発生: $e');
      if (e is DioException) {
        appLog('GroqService: Dioエラー詳細: ${e.response?.data}');
      }
  ...
  Future<String> summarize(String content) async {
    ...
    try {
          ...
          'messages': [
            {
              'role': 'system',
              'content':
                  'あなたは優秀な秘書です。提供された会議の文字起こしテキストを、以下の形式で構造化された議事録にまとめてください。\n'
                  '1. 議題（タイトル）\n'
                  '2. 要約（3〜5文程度）\n'
                  '3. 決定事項（箇条書き）\n'
      ...
      if (response.statusCode == 200) {
        ...
        appLog('GroqService: AI要約成功');
        return text;
      } else {
        appLog('GroqService: AI要約エラー: ${response.data}');
        throw Exception('AI要約に失敗しました: ${response.statusMessage}');
      }
    } catch (e) {
      appLog('GroqService: AI要約中に例外発生: $e');
